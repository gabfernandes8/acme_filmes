...
const insertbl_atores_nacionalidadecionalidadeAtor = async (dadosNacionalidadeAtor) => {
    ...
    try {
        let sql = `insert into tbl_atores_nacionalidade (nacionalidade_id, atores_id) values (${dadosNacionalidadeAtor.nacionalidade_id}, ${dadosNacionalidadeAtor.ator_id})`
...
const updateNacionalidadeAtor = async (dadosNacionalidadeAtor, idNacionalidadeAtor) => {
    ...
    try {
        let sql = `update tbl_atores_nacionalidade set nacionalidade_id = ${dadosNacionalidadeAtor.nacionalidade_id}, atores_id = ${dadosNacionalidadeAtor.atores_id} where id = ${idNacionalidadeAtor}`   
        let resultStatus = await prisma.$executeRawUnsafe(sql)
        if(resultStatus)
            return true
        else
...
const selectAllNacionalidadesByIdAtor = async (id) => {
    ...
    try {
        let sql = `select tbl_nacionalidade.id, tbl_nacionalidade.nome as nacionalidade from tbl_atores_nacionalidade inner join tbl_nacionalidade on tbl_atores_nacionalidade.nacionalidade_id=tbl_nacionalidade.id where tbl_atores_nacionalidade.atores_id = ${id}`
        let rsNacionalidadeAtor = await prisma.$queryRawUnsafe(sql)
        return rsNacionalidadeAtor
    } catch (error) {
        return false
